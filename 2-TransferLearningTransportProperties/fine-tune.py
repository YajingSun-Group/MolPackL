...
from model import build_fine_tune_model, freeze_layer
...
def main():
    ...
    for target_label in target_label_list:
            ...
            # load base model
            raw_model = tf.keras.models.load_model('../1-BandGapPrediction/model_band_gap-final-2.keras')
            # we just need top 6 layers
            base_model = tf.keras.models.Sequential(raw_model.layers[:8])
            print(base_model.summary())
            print(raw_model.summary())

            base_model = freeze_layer(base_model=base_model)
            model = build_fine_tune_model(base_model=base_model,hidden_layers_num = [64,32],lr=0.0001)
            ...
            history = model.fit(train_x,
                        train_y, epochs=1000, batch_size=32,
            ...
            print(f'/n/n{target_label} Training Done!')
            ...
            # save model
            model.save(f'./model/{target_label}_fine_tune.keras')
            ...
            # save results
            train_result.to_csv(f'./result/train_result_{target_label}_fine_tune.csv',index=False)
            test_result.to_csv(f'./result/test_result_{target_label}_fine_tune.csv',index=False)
            val_result.to_csv(f'./result/val_result_{target_label}_fine_tune.csv',index=False)
