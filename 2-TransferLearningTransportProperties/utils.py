...
def load_data(target_label):
    # Load data from file
    data0 = np.load(f'../dataset/{target_label}_tf_HS0.npy').reshape(-1, 64,64, 1)
    data1 = np.load(f'../dataset/{target_label}_tf_HS1.npy').reshape(-1, 64,64, 1)
    data2 = np.load(f'../dataset/{target_label}_tf_HS_pro.npy').reshape(-1, 64,64, 1)
    labels = pd.read_csv(f'../dataset/{target_label}_label.csv')['label'].values

    group = pd.read_csv(f'../dataset/{target_label}_label.csv')['group'].values
    cod_id = pd.read_csv(f'../dataset/{target_label}_label.csv')['id'].values

    return data0, data1, data2, labels, group, cod_id
